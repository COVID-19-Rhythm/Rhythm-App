...
class AppDelegate: UIResponder, UIApplicationDelegate {
       ...
       // FirebaseApp.configure()
        ...
        healthStore.enableBackgroundDelivery(for: readType2!, frequency: .immediate) { success, error in
            if !success {
                ...
                guard let quantityType3 = HKObjectType.quantityType(forIdentifier: .heartRate) else {
    ...
    //            self.getHealthData(type: HKQuantityTypeIdentifier.heartRate, dateDistanceType: .Week, dateDistance: 1)
    //            print("YAH")
