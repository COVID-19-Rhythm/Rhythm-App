        ...
        healthStore.enableBackgroundDelivery(for: readType2!, frequency: .immediate) { success, error in
            if !success {
                ...
                //self.retrieveSleepAnalysis() { _ in
                    self.getHealthData(type: .stepCount, dateDistanceType: .Week, dateDistance: self.onboarding ? 7 : 30, endDate: Date()) { value in
                    ...
                    for type in self.readData {
                        self.getHealthData(type: type, dateDistanceType: .Week, dateDistance: self.onboarding ? 30 : 7, endDate: Date()) { value in
